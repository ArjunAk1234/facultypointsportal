...
// Global database connection (assuming 'db' and collection constants are defined elsewhere in your project)
// For this snippet to compile, you'd need to define these. Example:
...
// TeacherPointsSnapshot stores a teacher's points at the time of a reset
type TeacherPointsSnapshot struct {
	TeacherID   primitive.ObjectID `json:"teacher_id" bson:"teacher_id"`
	TeacherName string             `json:"teacher_name" bson:"teacher_name"`
	Points      int                `json:"points_at_reset" bson:"points_at_reset"` // Total points before reset
}

// PointResetLog records a global point reset event
type PointResetLog struct {
	ID                primitive.ObjectID      `json:"_id" bson:"_id"`
	ResetDate         time.Time               `json:"reset_date" bson:"reset_date"`
	PreviousResetDate *time.Time              `json:"previous_reset_date,omitempty" bson:"previous_reset_date,omitempty"` // NEW: Date of the last reset
	Teachers          []TeacherPointsSnapshot `json:"teachers" bson:"teachers"`
	// Optional: AdminUser primitive.ObjectID `json:"admin_user,omitempty" bson:"admin_user,omitempty"` // Example if you want to track who initiated the reset
}

// --- NEW GLOBAL CONSTANT ---
const pointResetLogCollection = "pointResetLogs" // New collection for point reset logs

// =======================================================================================================
// Your existing (and some updated) handler functions start here
// =======================================================================================================

// User struct (assuming it's defined elsewhere, adding for completeness)
...
// GetTopTeachers (leaderboard) now uses the 'point' field directly from the teacher collection.
func GetTopTeachers(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	// UPDATED: Pipeline to sort teachers directly by their 'point' field
	pipeline := mongo.Pipeline{
		{{"$sort", bson.M{"point": -1}}}, // Sort by the teacher's 'point' field in descending order
		{{"$limit", 10}},                 // Limit to top 10
		{{"$project", bson.M{ // Project to match LeaderboardEntry struct
			"_id":          "$_id",
			"teacher_name": "$name",
			"total_points": "$point", // Use the 'point' field directly
		}}},
	}
...
// GetFacultyDashboard is updated to use the 'point' field for the leaderboard.
func GetFacultyDashboard(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	...
	go func() {
		// UPDATED: Leaderboard pipeline to use the teacher's 'point' field directly
		leaderboardPipeline := mongo.Pipeline{
			{{"$sort", bson.M{"point": -1}}}, // Sort teachers by their 'point' field
			{{"$limit", 10}},                 // Limit to top 10
			{{"$project", bson.M{ // Project to match LeaderboardEntry struct
				"_id":          "$_id",
				"teacher_name": "$name",
...
// =======================================================================================================
// NEW HANDLER FUNCTIONS FOR POINT RESET START HERE
// =======================================================================================================

// ResetAllTeacherPoints resets the 'point' field for all teachers to 0
// and logs their points before the reset.
func ResetAllTeacherPoints(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second) // Increased timeout for bulk ops
	defer cancel()

	teacherCollection := db.Collection(teacherCollection)
	pointResetLogCollectionRef := db.Collection(pointResetLogCollection)

	// --- NEW LOGIC: Find the most recent previous reset date ---
	var lastResetLog PointResetLog
	opts := options.FindOne().SetSort(bson.D{{"reset_date", -1}}) // Sort by date descending to get the latest
	err := pointResetLogCollectionRef.FindOne(ctx, bson.M{}, opts).Decode(&lastResetLog)
	var previousResetDate *time.Time
	if err == nil { // Found a previous reset log
		previousResetDate = &lastResetLog.ResetDate
	} else if err != mongo.ErrNoDocuments {
		// Handle other potential errors during lookup
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to look up previous reset log: " + err.Error()})
		return
	}
	// --- END NEW LOGIC ---

	// 1. Fetch all teachers to record their current points
	cursor, err := teacherCollection.Find(ctx, bson.M{})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch teachers for point logging: " + err.Error()})
		return
	}
	defer cursor.Close(ctx)
	...
	if err := cursor.All(ctx, &teachers); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to decode teachers for point logging: " + err.Error()})
		return
	}

	if len(teachers) == 0 {
		c.JSON(http.StatusOK, gin.H{"message": "No teachers found to reset points for."})
		return
	}

	// 2. Create a snapshot of current points for logging
	var snapshots []TeacherPointsSnapshot
	for _, teacher := range teachers {
		snapshots = append(snapshots, TeacherPointsSnapshot{
			TeacherID:   teacher.ID,
			TeacherName: teacher.Name,
			Points:      teacher.Point, // Use the teacher's current 'point' field
		})
	}

	// 3. Log the reset event
	resetLog := PointResetLog{
		ID:                primitive.NewObjectID(),
		ResetDate:         time.Now(),
		PreviousResetDate: previousResetDate, // Populate the new field
		Teachers:          snapshots,
	}
	if _, err := pointResetLogCollectionRef.InsertOne(ctx, resetLog); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to log point reset event: " + err.Error()})
		return
	}

	// 4. Reset all teachers' 'point' field to 0
	_, err = teacherCollection.UpdateMany(ctx, bson.M{}, bson.M{"$set": bson.M{"point": 0}})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to reset teacher points: " + err.Error()})
		return
		// In a production system, you might want to consider rolling back the log entry here
		// or having a more robust error handling/retry mechanism.
	}

	c.JSON(http.StatusOK, gin.H{
		"message":      fmt.Sprintf("Successfully reset points for %d teachers. Previous points logged.", len(teachers)),
		"reset_log_id": resetLog.ID,
	})
}

// ListPointResetLogs retrieves and lists all historical point reset logs.
func ListPointResetLogs(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	pointResetLogCollectionRef := db.Collection(pointResetLogCollection)
	cursor, err := pointResetLogCollectionRef.Find(ctx, bson.M{}, options.Find().SetSort(bson.D{{"reset_date", -1}})) // Sort by most recent reset
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch point reset logs: " + err.Error()})
		return
	}
	defer cursor.Close(ctx)

	var logs []PointResetLog
	if err := cursor.All(ctx, &logs); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to decode point reset logs: " + err.Error()})
		return
	}

	if logs == nil { // Ensure an empty array is returned instead of nil for JSON consistency
		c.JSON(http.StatusOK, []PointResetLog{})
		return
	}
...
// GeneratePointResetReportCSV generates a CSV report for a specific point reset log.
func GeneratePointResetReportCSV(c *gin.Context) {
	logIDStr := c.Param("id")
	logID, err := primitive.ObjectIDFromHex(logIDStr)
	if err != nil {
	...
	pointResetLogCollectionRef := db.Collection(pointResetLogCollection)
	var resetLog PointResetLog
	err = pointResetLogCollectionRef.FindOne(ctx, bson.M{"_id": logID}).Decode(&resetLog)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			c.JSON(http.StatusNotFound, gin.H{"error": "Point reset log not found"})
		} else {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error fetching point reset log: " + err.Error()})
		}
		return
	}

	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=point_reset_report_%s.csv", resetLog.ResetDate.Format("20060102_150405")))
	writer := csv.NewWriter(c.Writer)

	// --- UPDATED HEADERS ---
	headers := []string{"Teacher ID", "Teacher Name", "Points Before Reset", "Reset Date", "Previous Reset Date"}
	_ = writer.Write(headers)

	// Determine string for previous reset date
	var prevResetDateStr string
	if resetLog.PreviousResetDate != nil {
		prevResetDateStr = resetLog.PreviousResetDate.Format(time.RFC3339) // Or "2006-01-02 15:04:05" for a more readable format
	} else {
		prevResetDateStr = "N/A (First Reset)"
	}

	for _, teacher := range resetLog.Teachers {
		row := []string{
			teacher.TeacherID.Hex(),
			teacher.TeacherName,
			strconv.Itoa(teacher.Points),
			resetLog.ResetDate.Format(time.RFC3339), // Or "2006-01-02 15:04:05"
			prevResetDateStr,                        // Add previous reset date
		}
		_ = writer.Write(row)
	}
	writer.Flush()
